...
echo [LICENSE NOTE] SegFormer-B3 is a separate comparison baseline only.
echo [LICENSE NOTE] Generated checkpoints/cache artifacts stay outside redistribution unless license terms are separately reviewed.
echo.

if not exist "seg\transformer_b3\hf_pretrained\segformer_b3_ade\config.json" (
  echo [WARN] HuggingFace SegFormer-B3 pretrained folder was not found.
  echo [INFO] Running setup first...
  call 02_SETUP_SEGFORMER_B3_HF.bat
  if errorlevel 1 (
    echo [ERROR] SegFormer-B3 setup failed.
