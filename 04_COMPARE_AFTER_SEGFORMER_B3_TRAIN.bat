...
echo [BASELINE NOTE] SegFormer-B3 is handled as a separated comparison baseline for LiteRaceSegNet.
echo [RELEASE NOTE] Downloaded weights, caches, and generated checkpoints stay in the local comparison environment.
...
if not exist "seg\transformer_b3\checkpoints\segformer_b3_best.pth" (
  ...
  echo Run TRAIN_SEGFORMER_B3_ONLY.bat first.
  pause
  exit /b 1
)
