...
def _finish_metrics(totals: Dict[str, int]) -> Dict[str, Optional[float]]:
  ...
  total = tp + tn + fp + fn
  if total == 0:
    return {"pixel_acc": None, "miou_binary": None, "iou_damage": None, "iou_background": None}
...
def _evaluate_if_possible(
  ...
  if not image_paths or not mask_dir:
    return {"pixel_acc": None, "miou_binary": None, "iou_damage": None, "iou_background": None, "eval_images": 0}
...
def main():
  ...
  p.add_argument("--latency_repeats", type=int, default=50, help="0 disables synthetic latency test")
