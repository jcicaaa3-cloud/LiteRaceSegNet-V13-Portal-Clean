...
def normalize_stem(stem: str) -> str:
  """Normalize names while preserving numbers and Japanese letters.
    ...
    for word in _TRAILING_MASK_WORDS:
      for pat in (f"_{word}", f"-{word}", f" {word}", word):
        if s.endswith(pat) and len(s) > len(pat):
          s = s[: -len(pat)].strip(" _-")
          changed = True
  # Keep letters/numbers/Japanese only; remove separators and punctuation.
  return "".join(re.findall(r"[0-9a-zぁ-んァ-ン一-龯ー]+", s))
...
def find_best_mask_for_image(
  ...
  *,
  ...
  # Keep separate from dangerous auto-pairing when two candidates are almost tied.
