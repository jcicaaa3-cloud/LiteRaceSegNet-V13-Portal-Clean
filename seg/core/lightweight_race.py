...
class LiteIRBlock(nn.Module):
  ...
  This custom block adapts MobileNetV2/V3-style inverted-residual ideas and keeps the inverted-residual
  design principle but uses a simplified channel/layout choice for this project.
  """
...
class LiteASPP(nn.Module):
  ...
  def __init__(self, in_ch, out_ch, rates=(1, 2, 4)):
    ...
    # Keep BatchNorm out after global 1x1 pooling; it can fail with
...
class LiteRaceSegNet(nn.Module):
  ...
  - The project contribution is the road-damage-specific detail/context/boundary
  ...
  def __init__(
    ...
    # the architecture diagram. For ablation, it can be replaced with zeros.
    fuse_in = b + c + 1
