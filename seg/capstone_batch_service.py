...
- LLM usage is optional and limited to verbalizing the produced JSON summary.
...
def main():
  ...
  p.add_argument("--allow_demo_fallback", action="store_true", help="Presentation only: allow model masks that are nearly empty/full to be replaced by CV demo masks. Keep OFF for research evidence.")
  args = p.parse_args()
  ...
  mode_note = {
    ...
    "checkpoint": str(ckpt),
    "warning": None,
    "research_integrity": "Model mode keeps original mask behavior unless --allow_demo_fallback is explicitly supplied.",
  }
  ...
  if use_model:
    ...
    if args.allow_demo_fallback:
      mode_note["warning"] = "Demo fallback was explicitly enabled. This output is a visual placeholder for service-flow checking."
      cmd.append("--fallback_to_mock_if_bad_mask")
    if args.no_card:
      cmd.append("--no_card")
    if args.no_boundary:
