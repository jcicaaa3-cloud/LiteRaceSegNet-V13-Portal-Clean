...
def _ordinary_road_texture_mask(img: Image.Image) -> np.ndarray:
  ...
  intentionally conservative so ordinary input photos stay separate from a full
  cyan carpet.
  """
  w, h = img.size
...
def _demo_mask_from_image_no_resize(img: Image.Image) -> np.ndarray:
  ...
  visualized in a road-management service. Read it as a visual explanation, with metrics handled in evidence tables.
...
def _recommendation(severity: str) -> str:
  return {
    "high": "High-risk damage candidate. Prioritize field inspection and repair review.",
    "medium": "Moderate damage candidate. Confirm the location and add it to the repair plan.",
    "low": "Minor damage candidate. Check the image and field context because false positives are possible.",
    "none": "No clear damage candidate was detected.",
  }[severity]
...
def _japanese_explanation(summary: dict) -> str:
  ...
  if severity == "high":
    level = "high risk"
    action = "prioritize field inspection and repair review"
  elif severity == "medium":
    level = "moderate"
    action = "confirm the candidate location and add it to the repair plan"
  elif severity == "low":
    level = "minor"
    action = "check the image and field context because false positives are possible"
  else:
    level = "no clear damage candidate"
    action = "continue routine monitoring"
  return (
    f"Estimated damage area is {ratio:.2f}%, and the damage level is {level}."
    f"Detected component count: {comp_count}. Recommended action: {action}."
  )
...
def process_one(image_path: Path, out_dir: Path, mask_path=None, mock: bool = False, make_card: bool = True, make_boundary: bool = True, fallback_to_mock_if_bad_mask: bool = False, min_area_pixels: int = 0, mask_source_label: str = 'prediction_mask'):
  start = time.perf_counter()
  img = _read_image(image_path)
  w, h = img.size
  if mask_path is not None:
    mask = _read_mask(mask_path, (w, h))
    mode = mask_source_label or 'prediction_mask'
  ...
  summary["auto_explanation_ko"] = _japanese_explanation(summary)
  summary["llm_role_note"] = "CV model or CV heuristic creates the mask and measurements; an optional LLM only verbalizes this summary when connected."
...
def main():
  ...
  parser.add_argument("--mask_source_label", default="prediction_mask", help="Label recorded in summary mode when --mask/--mask_dir is used, e.g. model_prediction or ground_truth_mask")
  ...
  if args.image:
    summary = process_one(
      ...
      mask_source_label=args.mask_source_label,
    )
  ...
  for idx, image_path in enumerate(image_list, 1):
    ...
    summaries.append(process_one(image_path=image_path, out_dir=out_dir, mask_path=mask_path, mock=args.mock, make_card=not args.no_card, make_boundary=not args.no_boundary, fallback_to_mock_if_bad_mask=args.fallback_to_mock_if_bad_mask, min_area_pixels=args.min_area_pixels, mask_source_label=args.mask_source_label))
