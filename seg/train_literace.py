...
This is intentionally separated from SegFormer-B3 training.
- This script trains the user's custom lightweight CNN-based model: LiteRaceSegNet.
- It does NOT create or update SegFormer-B3 checkpoints.
...
class BinaryPotholeDataset(Dataset):
  def __init__(self, cfg: Dict, split: str, augment: bool = False):
    ...
    self.aug_cfg = cfg.get("train", {}).get("augmentation", {})
  ...
  def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
    ...
    if self.augment:
      hflip_p = float(self.aug_cfg.get("horizontal_flip_p", 0.5))
      if hflip_p > 0 and random.random() < hflip_p:
        image = image[:, :, ::-1].copy()
        mask = mask[:, ::-1].copy()

      # Light normalized-domain augmentation. This is intentionally conservative:
      # it reduces overfitting to road texture/lighting without changing labels.
      brightness = float(self.aug_cfg.get("brightness_delta", 0.0))
      if brightness > 0:
        image = image + np.random.uniform(-brightness, brightness, size=(3, 1, 1)).astype(np.float32)

      contrast = float(self.aug_cfg.get("contrast_range", 0.0))
      if contrast > 0:
        factor = np.float32(np.random.uniform(max(0.1, 1.0 - contrast), 1.0 + contrast))
        mean = image.mean(axis=(1, 2), keepdims=True)
        image = (image - mean) * factor + mean

      noise_std = float(self.aug_cfg.get("gaussian_noise_std", 0.0))
      if noise_std > 0:
...
def update_metrics(logits: torch.Tensor, labels: torch.Tensor, totals: Dict[str, int], ignore_index: int = 255, threshold: Optional[float] = None) -> None:
  logits = F.interpolate(logits, size=labels.shape[-2:], mode="bilinear", align_corners=False)
  if threshold is not None and logits.shape[1] == 2:
    pred = (torch.softmax(logits, dim=1)[:, 1] >= float(threshold)).long()
  else:
...
def validate(model: nn.Module, loader: DataLoader, device, cfg, class_weights) -> Dict[str, float]:
  ...
  for batch in loader:
    ...
    val_threshold = cfg.get("val", {}).get("threshold", None)
    update_metrics(outputs["out"], y, totals, ignore_index=ignore_index, threshold=val_threshold)
