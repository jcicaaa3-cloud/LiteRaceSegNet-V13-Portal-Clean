...
- It prepares a cached SegFormer-B3 baseline for local comparison; pothole fine-tuning is handled separately.
...
Distribution note:
- SegFormer-B3 is handled as an optional comparison baseline for LiteRaceSegNet.
- Downloaded/cache artifacts, pretrained weights, and generated checkpoints stay in the local comparison environment until license and redistribution terms are reviewed.
...
def main() -> int:
  ...
  print("[LICENSE NOTE] SegFormer-B3 is an optional comparison baseline only.")
  print("[RELEASE NOTE] Downloaded/cache artifacts stay in the local comparison environment.")
  ...
  manifest = {
    ...
    "important_note": (
      ...
      "For pothole_binary, the 2-class segmentation head is fine-tuned on the pothole dataset."
    ),
    "license_warning": (
      "SegFormer-B3 is an optional comparison baseline only. "
      "Downloaded/cache artifacts, pretrained weights, and generated checkpoints stay in the local comparison environment "
      "unless license and redistribution terms are separately reviewed."
    ),
...
"""
  ...
  print("[NEXT] Use 04_COMPARE_AFTER_SEGFORMER_B3_TRAIN.bat for architecture/cost comparison.")
