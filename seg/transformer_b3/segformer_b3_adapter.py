...
class SegFormerB3(nn.Module):
  """HuggingFace SegFormer-B3 wrapper for binary road-damage segmentation.

  By default this builds a B3-sized SegFormer-B3 from config so the code can be
  ...
  def __init__(
    ...
    try:
      ...
      raise RuntimeError(
        "SegFormer-B3 requires the optional transformers dependency. "
        "Run 01_INSTALL_TRANSFORMER_OPTIONAL.bat or install requirements_transformer_optional.txt first."
      ) from exc
